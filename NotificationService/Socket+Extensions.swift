...
extension Socket {
	...
	private func _handleReceive(_ data: AnyMsgData) async throws {
		...
		if appState == .background {
			var dataArray = GroupStorage.shared.codable([AnyMsgData].self, for: .device(.anyMsgData)) ?? []
...
enum NotificationServiceExtensionReceiver {
	static func handleReceive(_ data: AnyMsgData, completion: @escaping (Result<Void, Error>) -> Void) {
		Task { @SocketActor in
			do {
