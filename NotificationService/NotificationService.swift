...
final class NotificationService: UNNotificationServiceExtension {
    ...
    private let logger = Logger(subsystem: "com.bubbly.app", category: "NotificationServiceExtension")
    ...
    override func didReceive(
        ...
        guard let bestAttemptContent else {
            logger.warning("Failed to create mutable notification content; falling back to original content.")
        ...
        processNotificationData(data) { [weak self] _ in
            if let bestAttemptContent = self?.bestAttemptContent {
                ...
                if let originalContent = self?.originalContent {
                    self?.logger.info("bestAttemptContent missing; using original content fallback.")
                    contentHandler(originalContent)
                } else {
                    self?.logger.error("Both bestAttemptContent and originalContent missing; using request content fallback.")
                contentHandler(request.content)
                }
            }
        }
	...
	override func serviceExtensionTimeWillExpire() {
		...
		if let contentHandler = contentHandler, let bestAttemptContent = bestAttemptContent {
			contentHandler(bestAttemptContent)
        } else if let contentHandler = contentHandler, let originalContent = originalContent {
            contentHandler(originalContent)
		}
	}
