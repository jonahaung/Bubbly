...
public struct MarkdownParser {
    ...
    public static func parse(_ markdown: String) -> [MarkdownElement] {
        ...
        for line in lines {
            ...
            if trimmedLine.hasPrefix("```") {
                handleCodeBlock(line: trimmedLine, inCodeBlock: &inCodeBlock, currentCodeBlock: &currentCodeBlock, currentLanguage: &currentLanguage, elements: &elements)
            } else if inCodeBlock {
        ...
        if inCodeBlock, !currentCodeBlock.isEmpty {
            elements.append(.codeBlock(language: currentLanguage, content: currentCodeBlock.trimmingCharacters(in: .whitespacesAndNewlines)))
        }

        return elements
    }

    private static func handleCodeBlock(line: String, inCodeBlock: inout Bool, currentCodeBlock: inout String, currentLanguage: inout String?, elements: inout [MarkdownElement]) {
        if inCodeBlock {
            elements.append(.codeBlock(language: currentLanguage, content: currentCodeBlock.trimmingCharacters(in: .whitespacesAndNewlines)))
            currentCodeBlock = ""
            currentLanguage = nil
    ...
    private static func parseOrderedListItem(_ line: String) -> (Int, String)? {
        ...
        if let match = orderedListItemRegex.firstMatch(in: line, range: NSRange(location: 0, length: nsLine.length)) {
    ...
    private static func parseInlineElements(_ text: String) -> [MarkdownElement] {
        ...
        // Parse mentions
        let mentionMatches = mentionRegex.matches(in: text, range: NSRange(location: 0, length: nsText.length))
        ...
        // Parse hashtags
        let hashtagMatches = hashtagRegex.matches(in: text, range: NSRange(location: 0, length: nsText.length))
