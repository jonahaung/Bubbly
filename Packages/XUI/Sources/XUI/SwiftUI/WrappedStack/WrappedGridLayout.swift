...
public struct WrappedGridLayout: Layout {
	...
	public init(alignment: Alignment = .center, spacing: CGFloat? = 2) {
	                          ...
	                          cache _: inout Void)
	{
		...
		for row in result.rows {
			...
			for index in row.range {
				let xPos = rowXOffset + row.frame.minX + row
					.xOffsets[index - row.range.lowerBound] + bounds.minX
				let rowYAlignment = (row.frame.height - subviews[index].sizeThatFits(.unspecified)
					.height) *
	...
	struct FlowResult {
		...
		init(in maxPossibleWidth: Double,
		     subviews: Subviews,
		     alignment _: Alignment,
		     spacing: CGFloat?)
		{
			var itemsInRow = 0
			var remainingWidth = maxPossibleWidth
			...
			func spacingBefore(index: Int) -> Double {
				...
				return spacing ?? subviews[index - 1].spacing.distance(
			...
			func finalizeRow(index: Int, idealSize _: CGSize) {
				...
				let ySpacing = spacing ?? ViewSpacing().distance(
					to: ViewSpacing(),
					along: .vertical
				)
