...
struct ZoomableModifier: ViewModifier {
	...
	func body(content: Content) -> some View {
		content
			...
			.simultaneousGesture(
				dragGesture,
				including: transform == .identity ? .subviews : .gesture
			)
			.gesture(doubleTapGesture.exclusively(before: magnificationGesture))
	}
...
public extension View {
	func zoomable(minZoomScale: CGFloat = 1,
	              doubleTapZoomScale: CGFloat = 2) -> some View
	{
	...
	func zoomable(minZoomScale: CGFloat = 1,
	              ...
	              outOfBoundsColor: Color) -> some View
	{
		ZStack {
			outOfBoundsColor
			self.zoomable(
