	...
	func body(content: Content) -> some View {
		ZStack(alignment: .top) {
			header()
				...
				.scaleEffect(
					scrollViewOffset < 0 ? (height - scrollViewOffset) / height : 1,
					anchor: .top
				)
				.animation(
					scrollViewOffset <= 10 ? .spring(duration: 0.5) : nil,
					value: scrollViewOffset
				)
			content
		}
	}
...
public extension View {
	func stretchyHeader(_ scrollViewOffset: Binding<CGFloat>,
	                    height: CGFloat,
	                    multiplier: CGFloat = 1,
	                    @ViewBuilder header: @escaping () -> some View) -> some View
	{
		modifier(
