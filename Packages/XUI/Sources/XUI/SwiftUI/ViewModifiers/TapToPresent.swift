...
public extension View {
    func presentSheet(@ViewBuilder content: @escaping () -> some View, onDismiss: sending (() -> Void)? = nil) -> some View {
        ModifiedContent(content: self, modifier: PresentSheetModifier(destination: content, onDismiss: onDismiss))
    }

    func presentFullScreen(@ViewBuilder content: @escaping () -> some View, onDismiss: sending (() -> Void)? = nil) -> some View {
        ModifiedContent(content: self, modifier: PresentFullScreenModifier(destination: content, onDismiss: onDismiss))
    }

    func sheetWithZoomTransition(id: String = UUID().uuidString, @ViewBuilder destination: @escaping () -> some View) -> some View {
