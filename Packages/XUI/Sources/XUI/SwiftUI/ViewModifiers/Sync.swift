...
public extension View {
	func debounceSync<Value: Equatable>(_ original: Binding<Value>,
	                                    _ changed: Binding<Value>,
	                                    _ seconds: Double = 0.5) -> some View
	{
		ModifiedContent(
			content: self,
			modifier: DebounceSync(original: original, changed: changed, seconds: seconds)
		)
	}

	func lazySync<Value: Equatable>(_ original: Binding<Value>,
	                                _ changed: Binding<Value>) -> some View
	{
		ModifiedContent(
			content: self,
			modifier: LazySynchronizer(original: original, changed: changed)
		)
	}
}
