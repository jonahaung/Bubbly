        ...
        ZStack {
            if let placeholder {
                if appearance == .inactive {
                    switch placeholder {
                    case .hiddenFrame:
                        destination(.init(storedAppearance: .active, storedAppearanceBinding: .constant(.active)))
                            .hidden()
                    }
                }
            } else {
...
extension View {
    ...
    public func destroyOnDisappear() -> some View {
...
public struct AnyLazyViewResolver {
    ...
    public func resolve<Content: View>(
        ...
        if let typed = resolved as? Content {
            ...
            assertionFailure("AnyLazyViewResolver returned a different view type than expected. Falling back to original content.")
            return content()
        }
    }
}
