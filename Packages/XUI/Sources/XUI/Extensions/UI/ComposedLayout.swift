...
public struct ComposedLayout: Layout {
	...
	public func makeCache(subviews: Subviews) -> Caches {
		Caches(
			topCache: hStack.makeCache(subviews: topViews(subviews: subviews)),
	                         ...
	                         cache: inout Caches) -> CGSize
	{
		...
		return CGSize(
			width: max(tSize.width, max(cSize.width, bSize.width)),
	                          ...
	                          cache: inout Caches)
	{
		...
		if !tViews.isEmpty {
			let tSize = hStack.sizeThatFits(
				proposal: proposal,
				subviews: tViews,
				cache: &cache.topCache
			)
			hStack.placeSubviews(
				in: bounds,
				proposal: proposal,
				subviews: tViews,
				cache: &cache.topCache
