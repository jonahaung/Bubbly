...
extension Crypto {
    ...
    static func sampleUsage1() -> Bool {
        ...
        let senderSymmetricKey = Crypto.generateSymmetricKeyBetween(
            AliceSender.privateKey,
            and: BobReceiver.publicKey,
            salt: salt
        )!
        ...
        let reveiverSymmetricKey = Crypto.generateSymmetricKeyBetween(
            BobReceiver.privateKey,
            and: AliceSender.publicKey,
            salt: salt
        )!
        let decryptedData = Crypto.decrypt(
            encryptedData: encryptedData,
            using: reveiverSymmetricKey
        )

        let bobSecret = String(data: decryptedData!, encoding: .utf8)!

        // The decripted data, should be equals with the secret
        return Crypto.dataPlainMessageToHumanFriendlyPlainMessage(decryptedData) == TestVars
            .secretPlain
    }
    ...
    static func sampleUsageX() -> Bool {
        // Sender: Generating symmetric key and encrpting data USING shared symmetric key
        let senderSymmetricKey = Crypto.generateSymmetricKeyBetween(
            TestVars.AliceSender.privateKey,
            and: TestVars.BobReceiver.publicKey,
            salt: TestVars.salt
        )!
        let encryptedData = Crypto.encrypt(
            data: TestVars.secretPlainData,
            using: senderSymmetricKey
        )!

        // Receiver: Generating symmetric key and decrypting data USING shared symmetric key
        let reveiverSymmetricKey = Crypto.generateSymmetricKeyBetween(
            TestVars.BobReceiver.privateKey,
            and: TestVars.AliceSender.publicKey,
            salt: TestVars.salt
        )!
        let decryptedData = Crypto.decrypt(
            encryptedData: encryptedData,
    ...
    static func testDataToStringConversions() -> Bool {
        let plainSecretUtf8Data = Crypto
            .humanFriendlyPlainMessageToDataPlainMessage(TestVars.secretPlain)

        let aliceEncryptedData = Crypto.encrypt(
            data: plainSecretUtf8Data!,
            sender: TestVars.AliceSender.privateKey,
            receiver: TestVars.BobReceiver.publicKey,
            salt: TestVars.salt
        )!

        let aliceEncryptedDataOverTheNetwork = Crypto
            .encondeForNetworkTransport(encrypted: aliceEncryptedData)

        guard let bobEncryptedData = Crypto
            .decodeFromNetworkTransport(string: aliceEncryptedDataOverTheNetwork)
        else { return false }

        let bobDecryptedData = Crypto.decrypt(
            encryptedData: bobEncryptedData,
            receiver: TestVars.BobReceiver.privateKey,
            sender: TestVars.AliceSender.publicKey,
            salt: TestVars.salt
        )
    ...
    static func testPublicKeysHotStorage() -> Bool {
        ...
        // Test : After fetching a stored Public Key, should have the same value that the key that
