...
public struct DeeplinkTelemetry: Sendable {
    ...
    public var onFailed: @Sendable (_ url: URL, _ error: DeeplinkParseError, _ metadata: Metadata) -> Void

    public init(
        onParsed: @escaping @Sendable (URL, Deeplink, Metadata) -> Void = { _,_,_ in },
        onFailed: @escaping @Sendable (URL, DeeplinkParseError, Metadata) -> Void = { _,_,_ in }
    ) {
