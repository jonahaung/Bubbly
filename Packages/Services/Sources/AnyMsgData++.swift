...
public extension AnyMsgData {
	...
	init?(userInfo: [AnyHashable: Any]) {
		...
		guard let object = try? JSONDecoder().decode(AnyMsgData.self, from: data) else {
			return nil
		}
		self = object
	}
	...
	private static func decrypt(_ payload: String) -> String? {
		guard let (salt64, publicKeyString, encryptedString) = CryptoService.shared.parsePayload(
			payload
		) else {
			return nil
		}
