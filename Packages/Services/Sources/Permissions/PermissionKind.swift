...
public extension PermissionKind {
	var permission: any Permission {
		...
		case .location(let access): LocationPermission(access: access)
		case .microphone: MicrophonePermission()
		case .photoLibrary: PhotoLibraryPermission()
		case .notification(let access): NotificationPermission(access: access)
...
extension PermissionKind: Hashable {
	public static func == (lhs: PermissionKind, rhs: PermissionKind) -> Bool {
		...
		case (.notification(let lhsAccess), .notification(let rhsAccess)): lhsAccess == rhsAccess
		case (.location(let lhsAccess), .location(let rhsAccess)): lhsAccess == rhsAccess
		default: false
		}
	}
