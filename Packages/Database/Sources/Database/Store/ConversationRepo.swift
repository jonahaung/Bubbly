...
public enum ConversationRepo {
	...
	static func resolveContactID(from conID: String) throws -> String {
		...
		guard let contactID = components.first, !contactID.isEmpty else { throw XError.invalidConversationID }
		return contactID
	}
	
	public static func search(from name: String, currentUserId: String) async throws -> Conversation? {
		if let contact = try await ContactRepo.search(named: name) { return Conversation(.contact(contact)) }
		if let group = try await ContactRepo.searchGroup(named: name) { return Conversation(.group(group)) }
		return nil
	}
}
