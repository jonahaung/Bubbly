...
extension StringMergable {
	public func mergedString(current: String, incoming: String) -> String {
...
extension PContact {
	...
	public convenience init(from item: Contact) {
		self.init(
			uid: item.id,
			name: item.name,
			mobile: item.mobile,
