...
extension ScrollReducer {
    func reduceGeometry(
        newValue: VScrollGeometry, paginationState: ScrollCoordinator.PaginationState
    ) -> Effect? {
        ...
        case let .dataUpdate(dataUpdate):
            ...
            case let .insert(edge, geometry):
                ...
                if edge == .top {
                    ...
                    return .endUpdate(
                        .insert(edge: edge, geometry: geometry), scrollItem: .y(y, .scroll)
                    )
                }
            ...
            case let .remove(edge, geometry):
                if edge == .top {
                    ...
                    return .endUpdate(
                        .remove(edge: edge, geometry: newGeometry), scrollItem: .y(y, .scroll)
                    )
                }
                return .endUpdate(.remove(edge: .bottom, geometry: newValue), scrollItem: nil)
            case let .append(id):
                return .endUpdate(
                    .append(msgID: id), scrollItem: .id(id, .animated(.easeOut(duration: 0.2)))
                )
