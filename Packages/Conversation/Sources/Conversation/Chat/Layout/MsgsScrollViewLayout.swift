...
extension MsgsScrollViewLayout {
    ...
    func sizeThatFits(
        ...
        return .init(width: size.width, height: max(size.height, cache.totalHeight), )
    }
    func placeSubviews(
        ...
        for (i, subview) in subviews.enumerated() {
            ...
            let absolutePosition = CGPoint(
                x: rect.minX + layout.position.x, y: rect.minY + layout.position.y, )
            subview.place(
                at: absolutePosition, anchor: value.anchor, proposal: ProposedViewSize(layout.size),
            )
        }
    }
}
...
extension MsgsScrollViewLayout {
    private func computeLayouts(subviews: Subviews) -> ([Cache.CellLayout], CGFloat) {
        ...
        for subview in subviews {
            ...
            layouts.append(.init(id: value.uid, size: size, position: .init(x: x, y: y)), )
...
extension MsgsScrollViewLayout {
    fileprivate func size(for subview: LayoutSubview, value: MsgLayoutValue) -> CGSize {
    ...
    fileprivate func measure(subview: LayoutSubview, value: MsgLayoutValue) -> CGSize {
        let ratio: CGFloat =
    ...
    fileprivate func sizeKey(for value: MsgLayoutValue) -> MsgsScrollViewLayout.SizeKey {
        let selected = value.uid == manager.selectedMsg?.id
        return .init(uid: value.uid, width: config.boundsWidth, selected: selected)
    }
    fileprivate func xPosition(for recipient: MsgRecipient) -> CGFloat {
        switch recipient {
    ...
    fileprivate func totalHeight(for layouts: [Cache.CellLayout]) -> CGFloat {
...
extension MsgsScrollViewLayout {
    private func makeSignature(subviews: Subviews) -> Int {
        ...
        for subview in subviews { hasher.combine(subview[MsgLayoutValueKey.self].id) }
