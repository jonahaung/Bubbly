...
final class MsgsScrollViewLayoutCache: @unchecked Sendable {
    ...
    init(maxSizeEntries: Int = 1200, maxLayoutEntries: Int = 8, ) {
        self.maxSizeEntries = maxSizeEntries
        self.maxLayoutEntries = maxLayoutEntries
        snapshot = .init(sizeStorage: [:], sizeOrder: [], layoutStorage: [:], layoutOrder: [], )
    }
    ...
    func invalidateSizes(for keys: [MsgsScrollViewLayout.SizeKey]) {
        ...
        defer { writeLock.unlock() }
        var new = snapshot
        for key in keys { new.sizeStorage.removeValue(forKey: key) }
    ...
    func invalidateLayouts(for signatures: [Int]) {
        ...
        defer { writeLock.unlock() }
        var new = snapshot
        for sig in signatures { new.layoutStorage.removeValue(forKey: sig) }
    ...
    @inline(__always) private func refresh<T: Equatable>(_ order: inout [T], _ key: T) {
