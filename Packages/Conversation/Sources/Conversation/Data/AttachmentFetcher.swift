...
public actor AttachmentFetcher {
    ...
    public typealias Completion =
        @Sendable (Result<AttachmentData, Error>) -> Void
    ...
    private var waiters:
        [QueueKey: [UUID: CheckedContinuation<AttachmentData, Error>]] = [:]
    ...
    private func waitForPending(
        ...
        return try await withThrowingTaskGroup(of: AttachmentData.self) {
            group in
            group.addTask {
                try await self.waitForPendingWithoutTimeout(key: key)
            }
            group.addTask {
    ...
    private func waitForPendingWithoutTimeout(key: QueueKey) async throws
        -> AttachmentData
    {
        let waiterID = UUID()
    ...
    private func cancel(_ key: QueueKey) {
        ...
        if let handlers = completions.removeValue(forKey: key) {
            let result = Result<AttachmentData, Error>.failure(
                CancellationError()
            )
            Task(priority: .utility) {
                for handler in handlers {
