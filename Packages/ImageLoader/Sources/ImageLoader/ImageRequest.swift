...
/// ```
public struct ImageRequest: CustomStringConvertible, @unchecked Sendable,
	ExpressibleByStringLiteral
{
	...
	/// The request options. For a complete list of options, see
	/// ``ImageRequest/Options-swift.struct``.
	...
	public var urlRequest: URLRequest? {
		switch ref.resource {
		case let .url(url): url.map { URLRequest(url: $0) } // create lazily
		case let .urlRequest(urlRequest): urlRequest
		case .closure: nil
		}
	}
	...
	public var url: URL? {
		...
		case let .urlRequest(request): request.url
		case .closure: nil
		}
	}
	...
	/// - parameters:
	///   - url: The request URL.
	///   - processors: Processors to be apply to the image. See <doc:image-processing> to learn
	/// more.
	...
	///
	/// By default, the pipeline makes full use of all of its caching layers. You can change this
	/// behavior using options. For example, you can ignore local caches using
	/// ``ImageRequest/Options-swift.struct/reloadIgnoringCachedData`` option.
	///
	/// ```swift
	...
	public struct Options: OptionSet, Hashable, Sendable {
		...
		public static let disableDiskCache: Options = [
		...
		public static let reloadIgnoringCachedData: Options = [
			.disableMemoryCacheReads,
			.disableDiskCacheReads,
		]
		...
		/// Perform data loading immediately, ignoring
		/// ``ImagePipeline/Configuration-swift.struct/dataLoadingQueue``. It
	...
	public struct ThumbnailOptions: Hashable, Sendable {
		...
		///
		/// This option performs slightly faster than
		/// ``ImageRequest/ThumbnailOptions/init(size:unit:contentMode:)``
		...
		{
			targetSize = .flexible(
				size: ImageTargetSize(size: size, unit: unit),
		...
		public func makeThumbnail(with data: Data) -> PlatformImage? {
			ImageLoader.makeThumbnail(data: data, options: self)
		}
	...
	var closure: (@Sendable () async throws -> Data)? {
		if case let .closure(closure, _) = ref.resource { return closure }
		return nil
	}
}
...
extension ImageRequest {
	...
	enum Resource: CustomStringConvertible {
		...
		var description: String {
			switch self {
			case let .url(url): "\(url?.absoluteString ?? "nil")"
			case let .urlRequest(urlRequest): "\(urlRequest)"
			case let .closure(_, id): id
			}
		}
		...
		var imageId: String? {
			...
			case let .urlRequest(urlRequest): urlRequest.url?.absoluteString
			case let .closure(_, id): id
			}
		}
	}
