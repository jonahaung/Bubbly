...
public final class DataLoader: DataLoading, @unchecked Sendable {
	...
	///
	/// For example, you can use it to log network requests using
	...
	@Sendable public static func validate(response: URLResponse) -> Swift.Error? {
		...
		return (200 ..< 300).contains(response.statusCode) ? nil : Error
			.statusCodeUnacceptable(response.statusCode)
	}
	...
	public func loadData(for request: URLRequest)
		-> AsyncThrowingStream<(Data, URLResponse), Swift.Error>
	{
	...
	public enum Error: Swift.Error, CustomStringConvertible {
		...
		public var description: String {
			...
			case let .statusCodeUnacceptable(code):
				"Response status code was unacceptable: \(code.description)"
			}
		}
	}
}
...
private final class _DataLoader: NSObject, URLSessionDataDelegate, @unchecked Sendable {
	...
	{
		(delegate as? URLSessionTaskDelegate)?.urlSession?(
			session,
			task: task,
	...
	func urlSession(_ session: URLSession,
	                ...
	                completionHandler: @Sendable @escaping (URLSession.AuthChallengeDisposition,
	                                                        URLCredential?) -> Void)
	{
		(delegate as? URLSessionTaskDelegate)?.urlSession?(
			session,
	...
	func urlSession(_ session: URLSession,
	                ...
	                completionHandler: @Sendable @escaping (URLSession.DelayedRequestDisposition,
	                                                        URLRequest?)
	                	-> Void)
	{
		(delegate as? URLSessionTaskDelegate)?.urlSession?(
