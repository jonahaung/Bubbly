...
public extension ImageProcessing where Self == ImageProcessors.Resize {
	...
	static func resize(width: CGFloat,
	                   unit: ImageProcessingOptions.Unit = .points,
	                   upscale: Bool = false) -> ImageProcessors.Resize
	{
	...
	///   - unit: Unit of the target size. By default, `.points`.
	///   - upscale: `false` by default.
	static func resize(height: CGFloat,
	                   unit: ImageProcessingOptions.Unit = .points,
	                   upscale: Bool = false) -> ImageProcessors.Resize
	{
	...
	static func sticker() -> ImageProcessors.Sticker {
		ImageProcessors.Sticker()
	}
}

public extension ImageProcessing where Self == ImageProcessors.Circle {
...
public extension ImageProcessing where Self == ImageProcessors.RoundedCorners {
	...
	///
	/// - important: In order for the corners to be displayed correctly, the image must exactly
	/// match the size
	/// of the image view in which it will be displayed. See ``ImageProcessors/Resize`` for more
	/// info.
	static func roundedCorners(radius: CGFloat,
	                           unit: ImageProcessingOptions.Unit = .points,
	                           border: ImageProcessingOptions.Border? = nil) -> ImageProcessors
		.RoundedCorners
	{
...
public extension ImageProcessing where Self == ImageProcessors.Anonymous {
	...
	static func process(id: String,
	                    _ closure: @Sendable @escaping (PlatformImage) -> PlatformImage?)
		-> ImageProcessors
		.Anonymous
	{
	...
	public extension ImageProcessing where Self == ImageProcessors.CoreImageFilter {
		/// Applies Core Image filter – `CIFilter` – to the image.
		///
		/// - parameter identifier: Uniquely identifies the processor.
		static func coreImageFilter(name: String, parameters: [String: Any],
		                            identifier: String) -> ImageProcessors.CoreImageFilter
		{
		...
		static func coreImageFilter(_ filter: CIFilter, identifier: String) -> ImageProcessors
			.CoreImageFilter
		{
	...
	public extension ImageProcessing where Self == ImageProcessors.GaussianBlur {
