...
public extension ImageProcessors {
	struct Sticker: ImageProcessing, Hashable, CustomStringConvertible {
		public func process(_ image: PlatformImage) -> PlatformImage? {
			...
			if let pngData = try? extractSticker(from: data).pngData() {
				...
				//							return
				//							platformImage.processed.byAddingRoundedCorners(radius: radius, backgroundColor: color)
				//						}
				//					}
...
public final class ColorExtractor {
	...
	public func extractColors(from data: Data) async -> PhotoColorScheme? {
		...
		let rgbSources: [vImage.PixelBuffer<vImage.PlanarF>] = try! vImage
...
private nonisolated extension ColorExtractor {
	...
	func allocateDistancesBuffer() {
		...
		distances = UnsafeMutableBufferPointer<Float>
			.allocate(capacity: ColorExtractor.dimension * ColorExtractor.dimension * k)
	}
	...
	func initializeCentroids() {
		...
		let tmp = UnsafeMutableBufferPointer(
			...
			count: ColorExtractor.dimension * ColorExtractor
				.dimension
		)
		for i in 1 ..< k {
			distanceSquared(
				x0: greenStorage.baseAddress!,
				x1: centroids[i - 1].green,
				y0: blueStorage.baseAddress!,
				y1: centroids[i - 1].blue,
				z0: redStorage.baseAddress!,
				z1: centroids[i - 1].red,
	...
	/// 3. For each centroid, populate the corresponding row in the distances matrix with the
	/// distance squared
	/// between it and each matrix.
	/// 4. Use the BNNS reduction `argMin` on the distances matrix to create a vector with
	/// pixel-count elements.
	/// Each element in the vector is the centroid that's the closest color to the corresponding
	/// pixel.
	/// 5. For each centroid, use BNNS gather to create a vector for each RGB channel of the pixel
	/// colors for that centroid. Compute the mean value of that vector and set the centroid color
	/// to that average.
		...
		for centroid in centroids.enumerated() {
			distanceSquared(
				...
				result: distances.baseAddress!
					.advanced(by: ColorExtractor.dimension * ColorExtractor
						.dimension * centroid.offset)
			)
		}
	}
	...
	private func distanceSquared(x0: UnsafePointer<Float>, x1: Float,
