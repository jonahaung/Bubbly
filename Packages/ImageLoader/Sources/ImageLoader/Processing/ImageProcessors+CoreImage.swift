...
// Copyright (c) 2015-2024 Alexander Grebenyuk (github.com/kean).
	...
	public extension ImageProcessors {
		...
		struct CoreImageFilter: ImageProcessing, CustomStringConvertible, @unchecked Sendable {
			...
			static func applyFilter(named name: String,
			                        parameters: [String: Any] = [:],
			                        to image: PlatformImage) throws -> PlatformImage
			{
			...
			public static func apply(filter: CIFilter,
			                         to image: PlatformImage) throws -> PlatformImage
			{
				...
				try filter.setValue(getCIImage(), forKey: kCIInputImageKey)
				guard let outputImage = filter.outputImage else {
					throw Error.failedToApplyFilter(filter: filter)
				}
				guard let imageRef = context.createCGImage(outputImage, from: outputImage.extent)
				else {
			...
			public enum Error: Swift.Error, CustomStringConvertible, @unchecked Sendable {
				...
				public var description: String {
					...
					case let .failedToCreateFilter(name, parameters):
						"Failed to create filter named \(name) with parameters: \(parameters)"
					case let .inputImageIsEmpty(inputImage):
						"Failed to create input CIImage for \(inputImage)"
					case let .failedToApplyFilter(filter):
						"Failed to apply filter: \(filter.name)"
					case let .failedToCreateOutputCGImage(image):
						"Failed to create output image for extent: \(image.extent) from \(image)"
					}
				}
			}
		}
