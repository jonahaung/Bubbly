...
public extension ImagePipeline {
	...
	///   is finished.
	@discardableResult nonisolated func loadImage(with url: URL,
	                                              completion: @MainActor @Sendable @escaping (_ result: Result<
	                                              	ImageResponse,
	                                              	Error
	                                              >) -> Void) -> ImageTask
	{
	...
	///   - completion: A closure to be called on the main thread when the request
	///   is finished.
	@discardableResult nonisolated func loadImage(with request: ImageRequest,
	                                              completion: @MainActor @Sendable @escaping (_ result: Result<
	                                              	ImageResponse,
	                                              	Error
	                                              >) -> Void) -> ImageTask
	...
	@discardableResult nonisolated func loadImage(with request: ImageRequest,
	                                              progress: (
	                                              	@MainActor @Sendable (_ response: ImageResponse?,
	                                              	                      _ completed: Int64,
	                                              	                      _ total: Int64) -> Void
	                                              )?,
	                                              completion: @MainActor @Sendable @escaping (_ result: Result<
	...
	///
	/// You can call ``loadImage(with:completion:)-43osv`` for the request at any point after
	/// calling
	/// ``loadData(with:completion:)-6cwk3``, the pipeline will use the same operation to load the
	/// data,
	...
	/// - parameters:
	///   - request: An image request.
	///   - progress: A closure to be called periodically on the main thread when the progress is
	/// updated.
	///   - completion: A closure to be called on the main thread when the request is finished.
	@discardableResult nonisolated func loadData(with request: ImageRequest,
	                                             progress progressHandler: (
	                                             	@MainActor @Sendable (_ completed: Int64,
	                                             	                      _ total: Int64)
	                                             		-> Void
	                                             )? = nil,
	                                             completion: @MainActor @Sendable @escaping (Result<
	                                    ...
	                                    >) -> Void) -> ImageTask
	{
		makeImageTask(with: request, isDataTask: isDataTask) { event, task in
			DispatchQueue.main.async {
				...
				switch event {
				case let .progress(value): progress?(nil, value)
				case let .preview(response): progress?(response, task.currentProgress)
