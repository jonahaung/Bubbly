...
public extension ImagePipeline.Delegate {
    ...
    func willCache(data: Data, image _: ImageContainer?, for _: ImageRequest, pipeline _: ImagePipeline, completion: @escaping (Data?) -> Void) {
        completion(data)
    }

    func shouldDecompress(response _: ImageResponse, for _: ImageRequest, pipeline: ImagePipeline) -> Bool {
        pipeline.configuration.isDecompressionEnabled
    }

    func decompress(response: ImageResponse, request _: ImageRequest, pipeline: ImagePipeline) -> ImageResponse {
