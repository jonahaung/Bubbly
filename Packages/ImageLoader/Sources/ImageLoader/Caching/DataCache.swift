...
public final class DataCache: DataCaching, @unchecked Sendable {
	...
	public let queue = DispatchQueue(
		label: "com.github.kean.Nuke.DataCache.WriteQueue",
		qos: .utility
	)
	...
	public convenience init(name: String,
	                        filenameGenerator: @escaping (String) -> String? = DataCache.filename(
	                        	for:
	                        )) throws
	{
	...
	private func perform(_: Staging.ChangeRemoveAll) {
	...
	private func perform(_ change: Staging.Change) {
		...
		case let .add(data):
			do {
				...
				guard error.code == CocoaError.fileNoSuchFile.rawValue,
				      error.domain == CocoaError.errorDomain else { return }
				try? FileManager.default.createDirectory(
					at: self.path,
					withIntermediateDirectories: true,
					attributes: nil
