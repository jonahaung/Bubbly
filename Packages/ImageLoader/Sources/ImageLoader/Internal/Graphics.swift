...
struct ImageProcessingExtensions {
	...
	func byAddingRoundedCorners(radius: CGFloat,
	                            border: ImageProcessingOptions.Border? = nil) -> PlatformImage?
	{
		guard let cgImage = image.cgImage else {
			return nil
		}
		guard let ctx = CGContext.make(cgImage, size: cgImage.size, alphaInfo: .premultipliedLast)
		else {
			return nil
		}
	...
	func byAddingRoundedCorners(radius _: CGFloat,
	                            backgroundColor: UIColor? = nil) -> PlatformImage?
	{
...
extension PlatformImage {
	...
	func decompressed(isUsingPrepareForDisplay: Bool) -> PlatformImage? {
		...
		return draw(
			inCanvasWithSize: cgImage.size,
			drawRect: CGRect(origin: .zero, size: cgImage.size)
		)
	}
}

private extension CGContext {
	static func make(_ image: CGImage, size: CGSize,
	                 alphaInfo: CGImageAlphaInfo? = nil) -> CGContext?
	{
		if let ctx = CGContext.make(
			image,
			size: size,
			alphaInfo: alphaInfo,
	...
	static func make(_ image: CGImage,
	                 size: CGSize,
	                 ...
	                 colorSpace: CGColorSpace) -> CGContext?
	{
		CGContext(
			...
			bytesPerRow: 0,
			space: colorSpace,
			bitmapInfo: (alphaInfo ?? preferredAlphaInfo(for: image, colorSpace: colorSpace))
				.rawValue
		)
	}
	...
	private static func preferredAlphaInfo(for image: CGImage,
	                                       colorSpace: CGColorSpace) -> CGImageAlphaInfo
	{
	...
	private extension CGSize {
		func rotatedForOrientation(_ imageOrientation: CGImagePropertyOrientation) -> CGSize {
			...
			case .left, .leftMirrored, .right, .rightMirrored:
				CGSize(width: height, height: width) // Rotate 90 degrees
...
func makeThumbnail(data: Data, options: ImageRequest.ThumbnailOptions,
                   scale: CGFloat = 1.0) -> PlatformImage?
{
	...
	guard let image = CGImageSourceCreateThumbnailAtIndex(source, 0, options as CFDictionary) else {
		...
		if let imageProperties = CGImageSourceCopyPropertiesAtIndex(
			source,
			0,
			nil
		) as? [AnyHashable: Any],
			let orientationValue =
			imageProperties[kCGImagePropertyOrientation as String] as? UInt32,
...
private func getMaxPixelSize(for source: CGImageSource,
                             options thumbnailOptions: ImageRequest.ThumbnailOptions) -> CGFloat
{
