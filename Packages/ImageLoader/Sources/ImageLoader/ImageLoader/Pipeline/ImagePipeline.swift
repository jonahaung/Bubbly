...
public final class ImagePipeline {
	...
	private let tasksFetchOriginalData: TaskPool<
		TaskFetchOriginalDataKey,
		(Data, URLResponse?),
		Error
	>
	                        ...
	                        delegate: (any ImagePipeline.Delegate)? = nil)
	{
		...
		rateLimiter = configuration.isRateLimiterEnabled ? RateLimiter() : nil
		self.delegate = delegate ?? ImagePipelineDefaultDelegate()
		(configuration.dataLoader as? DataLoader)?.prefersIncrementalDelivery = configuration
	...
	nonisolated func makeImageTask(with request: ImageRequest,
	                               isDataTask: Bool = false,
	                               onEvent: (@Sendable (ImageTask.Event, ImageTask) -> Void)? =
	                               	nil) -> ImageTask
	{
		let task = ImageTask(
			taskId: nextTaskId.incremented(),
			request: request,
	...
	func startImageTask(_ task: ImageTask, isDataTask: Bool) {
		...
		let worker = isDataTask ? makeTaskLoadData(for: task.request) :
			makeTaskLoadImage(for: task.request)
		tasks[task] = worker
			.subscribe(priority: task.priority.taskPriority, subscriber: task) { [weak task] in
				task?.process($0)
			}
		onTaskStarted?(task)
	}
	...
	func makeTaskFetchOriginalImage(for request: ImageRequest) -> AsyncTask<ImageResponse, Error>
		.Publisher
	{
	...
	func makeTaskFetchOriginalData(for request: ImageRequest)
		-> AsyncTask<(Data, URLResponse?), Error>.Publisher
	{
		tasksFetchOriginalData.publisherForKey(TaskFetchOriginalDataKey(request)) {
			request.closure == nil ? TaskFetchOriginalData(self, request) : TaskFetchWithClosure(
				self,
				request
			)
		}
