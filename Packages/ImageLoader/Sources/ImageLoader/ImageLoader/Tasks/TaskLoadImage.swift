...
final class TaskLoadImage: AsyncPipelineTask<ImageResponse> {
	...
	private func fetchImage() {
		...
		if let processor = request.processors.last {
			...
			dependency = pipeline.makeTaskFetchOriginalImage(for: request)
	...
	private func process(_ response: ImageResponse,
	                     isCompleted: Bool,
	...
	private func didFinishProcessing(result: Result<ImageResponse, ImagePipeline.Error>,
	                                 isCompleted: Bool)
	{
		switch result {
		...
		operation = pipeline.configuration.imageDecompressingQueue.add { [weak self] in
			...
			let response =
				signpost(isCompleted ? "DecompressImage" : "DecompressProgressiveImage") {
					self.pipeline.delegate.decompress(
						response: response,
	...
	private func shouldStoreResponseInDataCache(_ response: ImageResponse) -> Bool {
		guard !response.container.isPreview,
		      ...
		      !(request.url?.isLocalResource ?? false)
		else {
			return false
		}
