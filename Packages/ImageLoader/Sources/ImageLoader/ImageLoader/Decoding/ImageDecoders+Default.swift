	...
	final class Default: ImageDecoding, @unchecked Sendable {
		...
		public func decodePartiallyDownloadedData(_ data: Data) -> ImageContainer? {
			...
			if assetType == .gif { // Special handling for GIF
				if !isPreviewForGIFGenerated, let image = ImageDecoders.Default._decode(
					data,
					scale: scale
				) {
			...
			guard let image = ImageDecoders.Default._decode(data[0 ... endOfScan], scale: scale)
			else {
				return nil
			}
			return ImageContainer(
				...
				type: assetType,
				isPreview: true,
				userInfo: [.scanNumberKey: numberOfScans]
			)
		}
	}
}
