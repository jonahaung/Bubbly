...
final class TaskFetchOriginalData: AsyncPipelineTask<(Data, URLResponse?)> {
	...
	private func loadData(urlRequest: URLRequest, finish: @escaping () -> Void) {
		...
		if pipeline.configuration.isResumableDataEnabled,
		   let resumableData = ResumableDataStorage.shared.removeResumableData(
		   	for: request,
		   	namespace: pipeline.id
		   )
		{
		...
		let task = Task { @ImagePipelineActor in
			...
			signpost(
				...
				"LoadImageData",
				.end,
				"Finished with size \(Formatter.bytes(self.data.count))"
			)
	...
	private func dataTask(didReceiveData chunk: Data, response: URLResponse) {
		...
		if urlResponse == nil {
			...
			if let resumableData, ResumableData.isResumedResponse(response) {
				...
				signpost(
					...
					"LoadImageData",
					.event,
					"Resumed with data \(Formatter.bytes(resumedDataCount))"
				)
			}
		...
		let progress = TaskProgress(
			...
			total: response.expectedContentLength + resumedDataCount
		)
	...
	private func tryToSaveResumableData() {
		...
		{
			ResumableDataStorage.shared.storeResumableData(
				resumableData,
				for: request,
				namespace: pipeline.id
			)
