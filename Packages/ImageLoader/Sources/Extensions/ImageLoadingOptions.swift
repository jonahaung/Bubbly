...
public struct ImageLoadingOptions {
	...
	public var failureImage: PlatformImage?
		...
		func transition(for response: ResponseType) -> Transition? {
			...
			case .failure: failureImageTransition
	...
	public var processors: [any ImageProcessing] = []
		...
		public struct ContentModes {
			...
			public init(success: UIView.ContentMode,
		...
		public init(placeholder: UIImage? = nil,
		            ...
		            failureImageTransition: Transition? = nil,
		...
		public init(placeholder: NSImage? = nil,
		            transition: Transition? = nil,
		            failureImage: NSImage? = nil,
		            failureImageTransition: Transition? = nil)
		{
			self.placeholder = placeholder
			self.transition = transition
	...
	public struct Transition {
		var style: Style
			...
			public static func fadeIn(duration: TimeInterval,
			                          options: UIView
			                          	.AnimationOptions = .allowUserInteraction) -> Transition
			{
				Transition(style: .fadeIn(parameters: Parameters(
					duration: duration,
					options: options
				)))
			}
			...
			public static func custom(_ closure: @escaping (ImageDisplayingView, UIImage) -> Void)
				-> Transition
			{
			...
			/// Custom transition. Only runs when the image was not found in memory cache.
			public static func custom(_ closure: @escaping (ImageDisplayingView, NSImage) -> Void)
				-> Transition
			{
				Transition(style: .custom(closure))
			}
