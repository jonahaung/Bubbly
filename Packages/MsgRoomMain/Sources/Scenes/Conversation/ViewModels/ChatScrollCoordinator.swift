...
final class ChatScrollCoordinator: ErrorPresenter {
    ...
    private(set) var defaultAnimation: Animation?
    ...
    @ObservationIgnored private(set) var visibleMessageIDs = [String]()
    ...
    @ObservationIgnored private let updateCoalescer = PassthroughSubject<UUID, Never>()
...
extension ChatScrollCoordinator {
    ...
    func didChangeVisibleMessageIDs(_ ids: [String]) {
        visibleMessageIDs = ids
    }

    func didChangeInputAccessoryFrame(_ oldValue: CGRect, _ newValue: CGRect) {
        if inputAccessoryFrame == nil && oldValue.height == newValue.height || oldValue.width != newValue.width {
            inputAccessoryFrame = newValue
            if let id = visibleMessageIDs.last {
                scrollTarget.scrollTo(id: id, anchor: .bottom)
            }
        ...
        guard newValue.maxY < oldValue.maxY else {
            isInputFirstResponder = false
            return
        }
        isInputFirstResponder = true
        if currentScrolledPosition == .atBottom { return }
        let targetY = scrollGeometry.offsetY + (oldValue.maxY - newValue.maxY) + scrollGeometry.topInset
