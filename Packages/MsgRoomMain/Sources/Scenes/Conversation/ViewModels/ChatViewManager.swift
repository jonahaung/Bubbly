...
final class ChatViewManager: ErrorPresenter, ViewReloadable {
    ...
    init(_ data: ConversationInitializer.PrefetchedData) {
        ...
        attachmentFetcher = AsyncFetcher(fetch: { [weak self] id in
            ...
            guard let msg = await cellItems.viewModel(of: id)?.msg else {
...
extension ChatViewManager: ChatScrollManagerDelegate {
    ...
    func scrollManager(reloadScrollView _: ChatScrollManager) {
        layoutIfNeeded()
    }
}
...
extension ChatViewManager {
    ...
    func msgCellLayoutFor(_ msg: Message, cellItems: [MsgCellViewModel]) -> MsgCellLayout {
        ...
        if index > 0, index < cellItems.count - 1,
           let cached = scrollManager.layoutCache.msgCellLayout(
               for: msg.uid
           )
        {
            return cached
...
extension ChatViewManager: ScrollViewPrefetcherDelegate {
    ...
    func allIndices(for _: ScrollViewPrefetcher) -> Range<Int> {
        cellItems.indices
    }

    func prefetcher(_: ScrollViewPrefetcher, prefetchItemsAt indices: [Int]) {
    ...
    func prefetcher(_: ScrollViewPrefetcher, cancelPrefetchingFor indices: [Int]) {
        guard let attachmentFetcher else { return }
        let ids = indices.compactMap { cellItems[safe: $0] }.filter { $0.msg.attachment != nil }.map(\.id)
        Task {
