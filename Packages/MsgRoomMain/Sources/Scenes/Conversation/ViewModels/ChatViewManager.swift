...
final class ChatViewManager: ChatViewUpdaterDelegate, ErrorPresenter {
	...
	var cellItems = [(Int, MsgCellViewModel)]()
	...
	init(_ data: ConversationInitializer.PrefetchedData) {
		...
		cellItems = data.msgs.map(MsgCellViewModel.init).enumerated
	}
	deinit {
		Log("ChatViewManager deinit")
	}
}

extension ChatViewManager: ChatScrollManagerDelegate {
	var canLoadPrevious: Bool {
		!cellItems.contains(where: { $0.1.id == config.firstMsgID })
	}
	var canLoadMore: Bool {
		!cellItems.contains(where: { $0.1.id == config.lastMsgID })
	}
	var canAdjustWindows: Bool {
		cellItems.count > config.maxNumberOfMsgsToDisplay
	}
	var lastMsg: MsgSnapshot? { cellItems.last?.1.msg }
	var firstMsg: MsgSnapshot? { cellItems.first?.1.msg }
	...
	func scrollManager(_ manager: ChatScrollManager, loadPrevious msg: Database.MsgSnapshot) {
		// Mark state and trim current window first to keep memory bounded
		...
		manager.updateLoadingState(.insertingItems(.top))
		...
		Task { [weak self] in
			...
			do {
				...
				if self.cellItems.first?.1.id == expectedFirstID || self.scrollManager.updatingState.isUpdating {
	...
	func setCellItems(_ msgs: [MsgSnapshot]) {
		cellItems = msgs.map(MsgCellViewModel.init).enumerated
	}
}
...
extension ChatViewManager {
	func currentLayoutWidth() -> CGFloat {
		// Prefer the actual scroll container width sourced from ScrollGeometry
		// via ChatScrollManager. Fallback to bottomBarFrame.width if zero.
		let width = scrollManager.containerWidth
		return width > 0 ? width : scrollManager.bottomBarFrame.width
	}
}

extension ChatViewManager {
	func msgCellLayoutFor(_ msg: MsgSnapshot, _ index: Int) -> MsgCellLayout {
		if let cached = cache.layout.msgCellLayout(for: msg.uid) {
			return cached
		}
		let next = cellItems[safe: index + 1]?.1.msg
		let previous = cellItems[safe: index - 1]?.1.msg
...
extension ChatViewManager {
	...
	func setSelectedMsg(_ uid: String) {
		if eventsManager.selectedMsg != nil {
			eventsManager.updateSelectedMsg(nil)
			return
		}
		guard let index = cellItems.firstIndex(where: { $0.1.id == uid }) else { return }
		let next = cellItems[safe: index + 1]?.1.msg
		let previous = cellItems[safe: index - 1]?.1.msg
		let newValue: SelectedMsg? = SelectedMsg(id: uid, previous: previous?.uid, next: next?.uid)
