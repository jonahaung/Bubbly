...
extension MsgsScrollViewLayout {
	fileprivate func calculateCache(for subviews: Subviews) -> Cache {
	...
	fileprivate func getValidCachedLayout(for subviews: Subviews) -> Cache? {
	...
	fileprivate func calculateCellLayouts(for subviews: Subviews) -> ([CellLayout], CGFloat) {
	...
	fileprivate func calculateCellLayout(
		for _: LayoutSubview,
...
extension MsgsScrollViewLayout {
	fileprivate func getOrCalculateSize(for messageID: String, subview: LayoutSubview) -> CGSize {
	...
	fileprivate func calculateOptimalSize(for subview: LayoutSubview) -> CGSize {
...
extension MsgsScrollViewLayout {
	fileprivate func calculatePosition(
	...
	fileprivate func calculateXPosition(for recipient: MsgRecipient, bubbleWidth: CGFloat) -> CGFloat {
		switch recipient {
		case .send:
			// Right aligned
...
extension MsgsScrollViewLayout {
	fileprivate func calculateTotalHeight(sizes: [CGSize]) -> CGFloat {
...
extension MsgsScrollViewLayout {
	fileprivate func placeSubview(_ subview: LayoutSubview, with layout: CellLayout) {
		subview.place(
			at: layout.position,
			anchor: .topLeading,
